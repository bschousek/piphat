...
def setup_stepper(stepgenIndex, section, axisIndex=None,
    ...
    logging.debug("stepper %i setup" %stepgenIndex)
    # prufix = False
    # if prufix:
    ...
    logging.debug("stepper %i type %s axis %s hasmotion %s" %(stepgenIndex, stepgenType, enableIndex, hasMotionAxis))
    ...
    scalepin=hal.pins['%s.scale' %stepgen]
    scalepin.link(scale)

    velpin=hal.pins['%s.vel-cmd' %stepgen]
    vel=hal.newsig('%s-vel' %sigBase, hal.HAL_FLOAT)
    velpin.link(vel)
...
def create_temperature_control(name, section, thread, hardwareOkSignal=None,
    ...
    hal.addf(wcomp.name+".funct", thread)
