...
def setup_stepper(stepgenIndex, section, axisIndex=None,
    ...
    if not velocityControlled:
        ...
        #velCmd=hal.newsig('%s.joint-vel-cmd' % axis, hal.HAL_FLOAT)
        
        velCmd=hal.newsig('emcmot-%i-velocity-cmd' % axisIndex, hal.HAL_FLOAT)
        velCmd.link('%s.joint-vel-cmd' % axis)
        #velCmd.link('emcmot-%i-velocity-cmd' % axisIndex, hal.HAL_FLOAT)
        velCmd.link('%s.velocity-cmd' % stepgen)
